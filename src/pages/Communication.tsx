...
export default function Communication() {
  ...
  const [view, setView] = useState<'internal' | 'client'>('client');
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col animate-fade-in">
      ...
      <div className="flex-1 flex gap-6 min-h-0">
        {/* Conversation List */}
        <div className="w-80 flex flex-col border border-border rounded-xl bg-card overflow-hidden">
          <Tabs value={view} onValueChange={(v) => setView(v as 'internal' | 'client')} className="flex-1 flex flex-col">
                  ...
                  <div className="relative">
                    ...
                    <div
                      className={cn(
                        ...
                        platformColors[selectedConversation.platform_type]
                      )}
                    >
                      {(() => {
                        const Icon = platformIcons[selectedConversation.platform_type];
                        return <Icon />;
                      })()}
                    </div>
                  </div>
                  <div>
                    <h3 className="font-semibold">{selectedConversation.client?.company_name}</h3>
                    <p className="text-sm text-muted-foreground capitalize">
                      via {selectedConversation.platform_type}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex justify-start">
                    <div className="max-w-[70%] bg-muted rounded-2xl rounded-tl-sm px-4 py-2">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {formatDistanceToNow(new Date(selectedConversation.last_message_at), { addSuffix: true })}
                      </p>
                    </div>
                  </div>
                </div>
...
function ConversationItem({
  ...
  const Icon = platformIcons[conversation.platform_type];
    ...
    >
      <div className="relative shrink-0">
        ...
        <div
          className={cn(
            'absolute -bottom-0.5 -right-0.5 h-5 w-5 rounded-full flex items-center justify-center text-white',
            platformColors[conversation.platform_type]
          )}
        >
          <Icon />
        </div>
      ...
      {conversation.unread_count > 0 && (
        <Badge className="shrink-0 h-5 w-5 p-0 flex items-center justify-center rounded-full">
